  ...
  return (
    <main className="p-6 min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      {instructors.length === 0 ? (
        ...
        <>
          {/* Desktop Table - md+ */}
            <div className="hidden md:block bg-white dark:bg-gray-800 rounded-2xl shadow-sm overflow-hidden mb-4">
                ...
                <table className="w-full min-w-[600px]">
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {filteredInstructors.map((inst) => (
                      <tr
                        key={inst.id}
                        className="hover:bg-gray-50 dark:hover:bg-gray-600/30 transition-colors"
                      >
                        ...
                        <td className="px-6 py-4 text-right whitespace-nowrap">
                          <div className="flex justify-end items-center gap-2 md:gap-3 flex-wrap sm:flex-nowrap">
                            <button
                            ...
                            <button
                              ...
                              className="px-3 py-2 bg-[#1F3463] hover:bg-[#172a4d] text-white rounded-lg flex items-center gap-1 transition-colors whitespace-nowrap"
                            >
          ...
          <div className="md:hidden grid grid-cols-1 sm:grid-cols-2 gap-4">
            {filteredInstructors.map((inst) => (
              <div key={inst.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-4 flex flex-col justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">{inst.name}</h3>
                  <p className="text-gray-600 dark:text-gray-400 text-sm">{inst.email}</p>
                </div>
                <div className="mt-4 flex justify-end space-x-2">
                  <button onClick={() => handleEdit(inst)} title="Edit" className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded">
                    <FaEdit className="w-5 h-5 text-gray-600 dark:text-gray-400" />
                  </button>
                  <button onClick={() => { setDeleteInstructorId(inst.id); setConfirmModalOpen(true); }} title="Delete" className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded">
                    <FaTrash className="w-5 h-5 text-gray-600 dark:text-gray-400" />
                  </button>
                  <button onClick={() => { setSelectedInstructor(inst); setAssignModalOpen(true); }} className="px-3 py-1 bg-[#1F3463] hover:bg-[#172a4d] text-white rounded text-sm">
                    Assign
                  </button>
                </div>
              </div>
